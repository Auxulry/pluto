      ...
      <div className="container mx-auto ">
        ...
        <div className="mt-8 flex flex-col gap-4">
          {box?.docs?.length > 0 && box?.docs?.map((item) => (
            <div key={item.id} className={`flex flex-col rounded-lg border-2 p-4 ${item.rejectedReason.option !== "" ? "bg-gray-200" : "bg-white"}`}>
              <div className="flex items-center">
                ...
                <div className="flex-grow flex flex-col gap-2">
                  ...
                  {item.attachments.filter((e) => e.src === "").length > 0 && (
                    <ul className="list-disc flex gap-6">
                      <li className="list-none text-red-500 italic">Dokumen Tidak lengkap.</li>
                    </ul>
                  )}
                </div>
                <div className="flex gap-3">
                  <button
                    ...
                    {...(item.rejectedReason.option === "" && ({ onClick: () => router.push(`/completes-document/${box?.id}/document/${item.id}`) }))}
                  >
                    <IoEyeSharp/>
                  </button>
                  <button
